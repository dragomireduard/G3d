...
int main(int argc, char** argv)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (rotateLight) {
            float radius = 2.0f;
            float lightX = sin(glfwGetTime()) * radius; // Calculăm poziția luminii pe axa X în funcție de timp
            float lightZ = cos(glfwGetTime()) * radius; // Calculăm poziția luminii pe axa Z în funcție de timp
            lightPos = glm::vec3(lightX, lightPos.y, lightZ);
        }
        model = glm::translate(model, lightPos);
        model = glm::scale(model, glm::vec3(0.2f)); // Scalați lumina pentru a o face mai vizibilă
...
void renderScene(const Shader& shader)
{
    ...
    model1 = glm::mat4();
    model1 = glm::translate(model1,glm::vec3(2.0f, 3.0f, 3.0f));
    shader.SetMat4("model", model1);
    renderCube();

    model2 = glm::mat4();
    model2 = glm::translate(model2, glm::vec3(3.0f, 1.0f, 2.0f));
    shader.SetMat4("model", model2);
    renderCube();

    model3 = glm::mat4();
    model3 = glm::translate(model3, glm::vec3(5.0f, 2.0f, 4.0f));
...
void processInput(GLFWwindow* window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_L) == GLFW_PRESS)
        rotateLight = true; // Pornim rotația luminii la apăsarea tastei "L"

    if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
        rotateLight = false; // Oprim rotația luminii la apăsarea tastei "S"
}
